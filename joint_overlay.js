(function () {
  ...
  const overlays = {
    'elbow_knee.jpg': {
      file: 'elbow_knee.jpg', referenceWidth: 2048, referenceHeight: 1308,
      sourceShapeWidth: 2560, sourceShapeHeight: 1635,
      elbow: {
    ...
    'wrist_ankle.jpg': {
      file: 'wrist_ankle.jpg', referenceWidth: 2048, referenceHeight: 1175,
      sourceShapeWidth: 2560, sourceShapeHeight: 1469,
      // 腕关节：0°从左侧开始。
      elbow: {
        center: {x: 772, y: 804}, baseAngle: 180, labelOffsetY: -20,
        contour: [[604,738],[625,716],[678,704],[773,708],[872,718],[919,746],[936,785],[932,827],[909,865],[862,891],[800,905],[738,902],[680,885],[638,852],[612,814]]
      },
      // 踝关节：0°从右侧开始，与腕关节同向旋转。
      knee: {
    ...
    'forearm_crus_flexor.png': {
      ...
      knee: {
        ...
        useImageBoundary: false,
        contour: [[1601,624],[1608,572],[1634,529],[1677,500],[1733,486],[1798,483],[1859,497],[1908,526],[1937,566],[1949,615],[1947,666],[1929,716],[1893,757],[1845,784],[1785,801],[1724,797],[1671,778],[1632,746],[1608,706],[1598,664]]
      },
  ...
  function selectOverlay() {
    overlay = overlays[currentFileName] || null;
    return overlay;
  }
  ...
  function ensureElbowShapesScaled() {
    if (!overlay || !loadedImage || currentFileName !== overlay.file || scaledShapesFor === `${currentFileName}:${loadedImage.width}x${loadedImage.height}`) return;
    ...
    scaledShapesFor = `${currentFileName}:${loadedImage.width}x${loadedImage.height}`;
  }
  ...
  function drawOverlay() {
    if (!overlay || currentFileName !== overlay.file || !loadedImage) return;
    drawArrow(overlay.elbow, overlay.elbow.baseAngle + dialAngle, dialAngle);
    // 两侧保持各自的0°起点，并采用相同旋转方向：
    // 圆盘顺时针时两个箭头都顺时针，逆时针时两个都逆时针。
  ...
  function onDial(p) {
    selectOverlay();
    if (!overlay || currentFileName !== overlay.file) return false;
    const d = pt(overlay.dial);
